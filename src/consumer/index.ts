...
import { type compressor, decompress } from '../config/redis';
...
export class Consumer {
                ...
                async (message: ConsumeMessage | null) => {
                    if (message?.properties.contentEncoding) {
                        const content = await decompress(message?.content!, message.properties.contentEncoding).catch(
                            (error) => '{"error":"Failed to decompress message"}',
                        );
    ...
    public async queueStatus() {
        ...
        if (this.channel) {
            ...
            const queue = await this.channel.assertQueue(this.queue, options).catch((error) => {
